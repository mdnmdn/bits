# CoinGecko Spot Market Order REST API Documentation
...
To place orders using CoinGecko data, use the `bits` tool to fetch prices from CoinGecko, then route order execution through an actual exchange provider (Binance, Bitget, MEXC, WhiteBit, or Crypto.com).
