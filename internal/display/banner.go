...
func PrintWelcomeBox() {
	...
	_, _ = fmt.Fprintln(w, blank)
	printCmdRow(w, "bits auth", "# Set up your API key")
	printCmdRow(w, "bits price --ids bitcoin", "# Get BTC price")
	printCmdRow(w, "bits markets --total 100", "# Top 100 by mkt cap")
	printCmdRow(w, "bits search ethereum", "# Search for a coin")
	printCmdRow(w, "bits trending", "# Trending coins")
	printCmdRow(w, "bits history bitcoin --days 30", "# 30-day price history")
	printCmdRow(w, "bits top-gainers-losers", "# Top gainers (paid)")
	printCmdRow(w, "bits watch --ids bitcoin", "# Live prices (paid)")
	printCmdRow(w, "bits tui markets", "# Interactive TUI")
	_, _ = fmt.Fprintln(w, blank)
	_, _ = fmt.Fprintln(w, sep)
	_, _ = fmt.Fprintln(w, blank)
