...
// executeCommand runs LegacyCmd with the given args and captures stdout/stderr.
// It resets LegacyCmd args via t.Cleanup.
func executeCommand(t *testing.T, args ...string) (stdout, stderr string, err error) {
	...
	resetAllFlags(LegacyCmd)

	LegacyCmd.SetArgs(args)
	LegacyCmd.SilenceUsage = true
	LegacyCmd.SilenceErrors = true
	t.Cleanup(func() {
		LegacyCmd.SetArgs(nil)
		LegacyCmd.SilenceUsage = false
		LegacyCmd.SilenceErrors = false
	})
